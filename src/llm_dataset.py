...
class LLMDataset(MathExpressionDataset):
    def __init__(self, num_samples=10000, file_path="./data/dataset.txt", force_recreate=False, max_length=10):
    ...
    def _create_shifted_dataset(self, tokenizer: Tokenizer, max_length: int):
        dataset = []
        for sample in self.tokens[:100]:
            equal_index = (sample == tokenizer.equal_token_id).nonzero(as_tuple=True)[0].item()
            
            for i in range(equal_index+1, len(sample)+1):
                input_seq = self.get_tensor_with_default(sample, i-max_length, i, tokenizer.pad_token_id)
                output_seq = self.get_tensor_with_default(sample, i+1-max_length, i+1, tokenizer.pad_token_id)
    ...
    def get_tensor_with_default(self, tens, lower, upper, default_value=0):
        ...
        valid_indices = [(tens[idx] if 0 <= idx < tens_size else default_value) for idx in indices]
