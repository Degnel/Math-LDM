...
class MathExpressionDataset(torch.utils.data.Dataset):
    def __init__(self, num_samples=10000, file_path="./data/dataset.txt", force_recreate=False):
        self.num_samples = num_samples
        self.file_path = file_path

        if force_recreate or not os.path.exists(file_path) or self._file_size() < num_samples:
    ...
    def _generate_and_save_data(self, force_recreate):
        ...
        num_to_generate = self.num_samples if force_recreate else self.num_samples - self._file_size()
        with open(self.file_path, "a" if not force_recreate else "w") as f:
